...
export class PlatformClient {
  ...
  /**
   * Sync anonymous ratings to the platform.
   * Strips all user identifiers — only model, taskType, rating, timestamp.
   */
  async syncRatings(ratings: Array<{ model: string; taskType: string; rating: number; timestamp: string }>): Promise<void> {
    await this.request('/api/mcp/ratings/sync', {
      method: 'POST',
      body: { ratings },
    });
  }
